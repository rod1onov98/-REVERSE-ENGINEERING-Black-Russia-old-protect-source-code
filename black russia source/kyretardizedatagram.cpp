...
uint8_t* unKyretardizeDatagram(unsigned char *buf, int length, int port, int unk) {
    int num1 = buf[0];
    int num2 = 0;
    int num3 = port ^ 204;
    int num4 = 0;
    ...
    for (int index = 0; index < length; ++index) {
        if (num4 == 0) {
            ba[index] = num4 ^ buf[index + 1];
            ++num4;
        } else {
            ba[index] = (num3 ^ buf[index + 1]) & 0xFF;
            --num4;
        }
        ba[index] = sampDecrTable[ba[index]];
        num2 ^= ba[index] & 170;
    }

    if (num2 != num1)
        printf("[WARNING] Invalid checksum: bChecksum " + intToString(num2) + " != bChecksumEncr " + intToString(num1));

    printf("DECRYPT:" + ByteArrayToHexString(ba, length));
    return ba;
}
